...
from native_extractor import import_via_native_extractor
...
def import_audio_from_url(url: str, tmpdir: str) -> ImportResult:
    ...
    if newpipe_enabled():
        print(
            f"[media-import] strategy=newpipe_native platform={platform} (yt-dlp skipped)",
            flush=True,
        )
        try:
            if platform == "youtube":
                result_tuple = import_youtube_via_newpipe(url, tmpdir)
                backend = "newpipe_extractor"
            else:
                result_tuple = import_via_native_extractor(url, tmpdir, platform)
                backend = "native_extractor"
            print(f"[media-import] success via {backend}", flush=True)
            return _import_result_from_tuple(*result_tuple)
        except Exception as e:
            print(f"[media-import] newpipe/native failed: {_format_error(e)}", flush=True)
            raise RuntimeError(f"Import failed ({platform}): {_format_error(e)}") from e
